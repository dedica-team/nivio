...
import { onTouchCancel, onTouchEnd, onTouchMove, onTouchStart } from './interactions-touch';
...
export default class ReactSVGPanZoom extends React.Component {
  constructor(props, context) {
    ...
    let defaultValue;
    if (viewBox) {
      const SVGMinX = viewBox.x;
      const SVGMinY = viewBox.y;
  ...
  pan(SVGDeltaX, SVGDeltaY) {
    let nextValue = pan(this.getValue(), SVGDeltaX, SVGDeltaY, null);
    this.setValue(nextValue);
  }
  ...
  render() {
    ...
    const touchAction =
      this.props.detectPinchGesture || [TOOL_PAN, TOOL_AUTO].indexOf(this.getTool()) !== -1
        ? 'none'
        : undefined;
    ...
    return (
      ...
      >
        ...
        >
          ...
          <g transform={toSVG(value)} style={blockChildEvents ? { pointerEvents: 'none' } : {}}>
            <rect
