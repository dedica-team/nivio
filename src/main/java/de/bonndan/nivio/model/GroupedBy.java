...
public class GroupedBy {
    ...
    public static GroupedBy newFunc(Function<Item, Map<String, List<StatusValue>>> newSupplier, Collection<Item> items) {
        var groups = new GroupedBy();
        items.forEach(item -> {
            StatusValue summary = StatusValue.summary(item.getAssessmentIdentifier(), newSupplier.apply(item).get(item.getAssessmentIdentifier()));
            groups.add(summary.getStatus().toString(), item);
        });
        return groups;
    }
