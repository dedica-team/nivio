...
class PersistentVolumeClaimDetailsTest {
    ...
    void testGetExtendedDetailsClassCastException() {
        ...
        Logger detailLogger = (Logger) LoggerFactory.getLogger(PersistentVolumeClaimDetails.class);
        ListAppender<ILoggingEvent> listAppender = new ListAppender<>();
        listAppender.start();
        detailLogger.addAppender(listAppender);
        var serviceDetails = new PersistentVolumeClaimDetails(new DefaultDetails());
        serviceDetails.getExtendedDetails(Map.of(), itemAdapter);
        List<ILoggingEvent> logsList = listAppender.list;
        var className = itemAdapter.getClass().getCanonicalName();
        assertThat(logsList.get(0).getMessage()).isEqualTo("class " + className + " cannot be cast to class de.bonndan.nivio.input.kubernetes.itemadapters.PersistentVolumeClaimItemAdapter (" + className + " and de.bonndan.nivio.input.kubernetes.itemadapters.PersistentVolumeClaimItemAdapter are in unnamed module of loader 'app')");
